...
internal sealed class SpecialDayVipSuppressionService
{
    ...
    {
        if (!player.IsValid || player.IsBot || _savedStates.ContainsKey(player.Slot)) return;
        if (!EnsureApi()) return;
        ...
        try
        {
            ...
            foreach (var entry in features)
            {
                ...
                if (state is null || !string.Equals(state.ToString(), "Enabled", StringComparison.OrdinalIgnoreCase)) continue;
    ...
    public void Restore(CCSPlayerController? player)
    {
        ...
        {
            try { _setPlayerFeatureState?.Invoke(_api, [player, feature.Name, feature.State]); }
            catch { }
        }
    }
