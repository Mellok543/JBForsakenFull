...
public sealed class JBFAdmin : BasePlugin
{
    ...
    public void OnLrStopCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (_lrApi is null)
        {
            Reply(command, "LR API недоступно.");
            return;
        }

        if (!_lrApi.TryEndActive())
        {
            Reply(command, "Активного LR сейчас нет.");
            return;
        }
    ...
    public void OnSpecialDayStopCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        {
            Reply(command, "Special Days API недоступно.");
            return;
        }

        if (!specialDays.TryStop())
        {
            Reply(command, "Активного или назначенного игрового дня нет.");
            return;
        }
    ...
    public void OnWardenRemoveCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        var warden = wardenApi?.Warden;
        if (wardenApi is null)
        {
            Reply(command, "Warden API недоступно.");
            return;
        }

        if (warden is null || !wardenApi.TryResign(warden))
        {
            Reply(command, "Командира сейчас нет.");
            return;
        }
    ...
    public void OnShopReloadCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if (_shopApi is null)
        {
            Reply(command, "Shop API недоступно.");
            return;
        }
    ...
    {
        if (!TryGetShopAndTarget(player, command, out var shop, out var target))
        {
            return;
        }

        if (!shop.TryGetCredits(target, out var credits))
        {
            Reply(command, "Не удалось получить SteamID игрока.");
            return;
        }
    ...
    public void OnCreditsGiveCommand(CCSPlayerController? player, CommandInfo command)
    {
        ChangeCredits(player, command, CreditOperation.Give);
    }

    [ConsoleCommand("css_jbf_credits_take", "Take JBF credits from a player")]
    [RequiresPermissions(AdminPermission)]
    [CommandHelper(2, "<игрок> <количество>", CommandUsage.CLIENT_AND_SERVER)]
    public void OnCreditsTakeCommand(CCSPlayerController? player, CommandInfo command)
    {
        ChangeCredits(player, command, CreditOperation.Take);
    }

    [ConsoleCommand("css_jbf_credits_set", "Set a player's JBF credit balance")]
    [RequiresPermissions(AdminPermission)]
    [CommandHelper(2, "<игрок> <количество>", CommandUsage.CLIENT_AND_SERVER)]
    public void OnCreditsSetCommand(CCSPlayerController? player, CommandInfo command)
    {
        ChangeCredits(player, command, CreditOperation.Set);
    }
    ...
    public void OnStatusCommand(CCSPlayerController? player, CommandInfo command)
    {
        ...
        Reply(command,
            $"Core: {FormatAvailability(core)} | Раунд: {core?.RoundState.ToString() ?? "нет API"} | " +
            $"LR: {FormatActivity(_lrApi?.IsActive, _lrApi?.ActiveGameName)} | " +
            $"Игровой день: {FormatActivity(specialDays?.IsActive, specialDays?.ActiveDayName)} | " +
            $"Shop: {FormatAvailability(_shopApi)}.");
    }

    private void ChangeCredits(CCSPlayerController? caller, CommandInfo command, CreditOperation operation)
    {
        if (!TryGetShopAndTarget(caller, command, out var shop, out var target))
        {
            return;
        }
        ...
        {
            Reply(command, "Количество должно быть целым неотрицательным числом.");
            return;
        }
        ...
        if (!success)
        {
            Reply(command, "Не удалось изменить баланс: SteamID игрока недоступен.");
            return;
        }

        var action = operation switch
        {
            CreditOperation.Give => $"выдал {amount}",
            CreditOperation.Take => $"снял {amount}",
            CreditOperation.Set => $"установил баланс {amount}",
            _ => "изменил баланс"
        };

        Reply(command, $"Баланс {target.PlayerName}: {balance} кредитов.");
        LogAction(caller, $"{action} кредитов игроку {target.PlayerName} ({GetSteamId(target)}); новый баланс: {balance}");
    }

    private bool TryGetShopAndTarget(
        ...
        out CCSPlayerController target)
    {
        ...
        var targets = command.GetArgTargetResult(1).Players
            .Where(candidate => candidate.IsValid && !candidate.IsBot)
            .ToArray();

        if (targets.Length == 0)
        {
            Reply(command, "Игрок не найден.");
            shop = null!;
            target = null!;
            return false;
        }
        ...
        if (targets.Length > 1)
        {
            Reply(command, "Найдено несколько игроков. Укажите имя точнее или используйте #userid.");
            shop = null!;
            target = null!;
            return false;
        }
        ...
        {
            Reply(command, "У вас недостаточно иммунитета для управления этим игроком.");
            shop = null!;
            target = null!;
            return false;
        }
    ...
    private static string FormatAvailability(object? api)
    {
        return api is null ? "недоступен" : "доступен";
    }

    private static string FormatActivity(bool? active, string? name)
    {
    ...
    {
        command.ReplyToCommand(JailbreakChat.Format(message));
    }

    private static void LogAction(CCSPlayerController? caller, string action)
    {
        var actor = caller is null
            ? "Server Console"
            : $"{caller.PlayerName} ({GetSteamId(caller)})";
    ...
    {
        return player.AuthorizedSteamID?.SteamId64.ToString() ?? "SteamID unavailable";
    }

    private enum CreditOperation
    {
        Give,
        Take,
        Set
