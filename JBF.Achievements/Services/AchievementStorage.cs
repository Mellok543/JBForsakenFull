...
internal sealed class AchievementStorage : IDisposable
{
        ...
        try
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                while (reader.Read())
                {
                    ...
                    var state = GetOrCreate(result, steamId);
                    state.Stats[reader.GetString(1)] = Math.Max(0, reader.GetInt64(2));
                }
            }
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                {
                    var steamId = reader.GetFieldValue<ulong>(0);
                    GetOrCreate(result, steamId).Unlocked.Add(reader.GetString(1));
                }
            }
        }
        catch (Exception ex)
    ...
    public void Dispose()
    {
        ...
        while (!_worker.IsCompleted && DateTime.UtcNow < deadline)
            Thread.Sleep(20);
    ...
    private async Task ProcessWritesAsync()
    {
        try
        {
            await foreach (var write in _writes.Reader.ReadAllAsync(_shutdown.Token))
            {
                ...
                {
                    if (await TryWriteAsync(write, _shutdown.Token))
                        break;
    ...
    private async Task<bool> TryWriteAsync(StorageWrite write, CancellationToken cancellationToken)
    {
        try
        {
            ...
            switch (write)
            {
                case StatWrite stat:
                    command.CommandText = $"""
                        INSERT INTO `{_statsTable}` (steam_id, stat_key, value)
                        VALUES (@steamId, @key, @value)
                ...
                case UnlockWrite unlock:
                    command.CommandText = $"""
                        INSERT IGNORE INTO `{_unlocksTable}` (steam_id, achievement_id)
                        VALUES (@steamId, @achievementId);
                        """;
    ...
    {
        if (string.IsNullOrWhiteSpace(prefix)) return "jbf_";
        return prefix.All(ch => char.IsLetterOrDigit(ch) || ch == '_') ? prefix : "jbf_";
    }
